...
def get_columns_database_types(id_db: int, table_name: str) -> dict[str, str]:
    """
    ...
    # Get database path
    src_db_path = get_path_database(id_db)
    if not src_db_path:
    ...
    return (columns, 200, "columns_types_obtained")
...
def create_model(db, table, columns):
    ...
    types = {"INTEGER": "Integer", "VARCHAR": "String", "TINYINT": "Boolean"}
...
def copy_database_fc(src_db_path, dest_db_path, src_table, dest_columns, dest_table):
    ...
    srcEngine._metadata.tables[src_table].columns = [
        i
        for i in srcEngine._metadata.tables[src_table].columns
        if (i.name in dest_columns)
    ]
...
def csv_to_sql():
    ...
    try:
        engine = create_engine("mysql://root:Admin538*@localhost:3306/public")
    except:
